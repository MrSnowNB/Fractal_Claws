...
### [x] DONE — Step 10: ContextBudget graphify_repo() + gate

**Gate:** ✅ 4/4 graphify tests passed, full suite clean, ctx-cache.json valid.

| Ticket | Task | Status |
|---|---|---|
| STEP-10-A | `context_budget.py` graphify_repo() implementation | ✅ |
| STEP-10-B | `_detect_zone()` path-parts fix | ✅ |
| STEP-10-C | Gate run — 4/4 graphify tests green | ✅ |
| STEP-10-D | Full suite gate + GRAPHIFY_COMPLETE journal event + ctx-cache.json verify | ✅ |

---

### [ ] Step 11: Luffy Law Mechanical Enforcement
**Spec:** `AI-FIRST/STEP-11-LUFFY-LAW-ENFORCEMENT.md`  
**Files:** `agent/sequence_gate.py`, `agent/runner.py`, `tests/test_luffy_law.py`  

> **SCOPE NOTE:** Law enforcement in Fractal Claws must stay minimal.
> The check is: *did the agent write anything to scratch?* Not a full audit trail.
> REASONING/VERIFY section requirements belong in MUMPS_Bot, not here.
> Keep gates as data-layer constraints, not governance ceremony.

**Sub-tasks:**

| Sub-task | Task | Status |
|---|---|---|
| 11-A | Add `LawViolationError` + `assert_scratch_written()` (non-INIT event check) to `agent/sequence_gate.py` | ✅ DONE |
| 11-B | Hook Law §1 in `execute_ticket()` before `TICKET_CLOSED` write — call `gate.assert_scratch_written(ticket_id)` | ⬅ NEXT |
| 11-C | Hook Law §2: emit `SCRATCHPAD_READ` event in `drain()` on scratchpad load | [ ] |
| 11-D | Hook Law §3: emit `LAW3_CACHE_HIT` info event in `build_prompt()` on cache skip | [ ] |
| 11-E | `tests/test_luffy_law.py` — 6 green ✅ already written | ✅ DONE |
| 11-F | Full suite gate + journal with anchor + commit + push | [ ] |

**Gate:** `pytest tests/test_luffy_law.py -v` (6 tests) + `pytest tests/ -v` (0 failed)

---

### [ ] Step 12: child_runner.py — Spawn Primitive (THE EMERGENCE PRIMITIVE)
**Prerequisite:** STEP-11 complete.

> **This is the thesis.** Everything above is infrastructure. This step
> demonstrates complex behavior emerging from simple rules:
> one ticket spawns N children, children execute independently,
> results aggregate back to parent. No Laws needed at child level —
> the data structure enforces behavior.

**What it does:**
- `agent/child_runner.py` — reads ticket, calls one tool sequence, writes `TicketResult`, exits
- A depth-0 ticket spawns depth-1 children via `delegate_task()`
- Children cannot spawn (depth field gates this — structural constraint, not a Law)
- Parent aggregates child results into its own `TicketResult`
- One end-to-end test: 1 parent → 2 children → results aggregate → parent closes
...
**Active ticket:** STEP-11-B  
**Status:** ready — STEP-11-A complete, assert_scratch_written() live and relaxed (non-INIT check only)  
**Last action:** [13:32] Perplexity restored NEXT-STEPS.md; updated STEP-11 sub-task status  
**Blockers:** none  
**Next:** Hook Law §1 in `execute_ticket()` — call `gate.assert_scratch_written(ticket.ticket_id)` before writing TICKET_CLOSED event; run pytest; journal + commit
